<template>
    <f7-page @page:beforein="load">
        ...
        <div class="content">
            <div v-show="tab=='carList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的用车申请</span>
                        </li>
                        <li class="expense-right-des">
                            <span>申请用车事由：{{item.result}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>申请用车时间：{{moment(item.startTime).format('YYYY-MM-DD hh:mm')}} - {{moment(item.endTime).format('YYYY-MM-DD hh:mm')}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>用车时长：{{item.time}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>用车目的地：{{item.address}}</span>
                        </li>
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="carList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>用车审批</span>
                </div>
                <div id="carApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
export default {
    ...
    methods: {
        moment,
        changeClick(index){
            let obj1 = document.getElementById('carList');
            let obj2 = document.getElementById('carApply');
            if (index == 1){
                this.tab = "carList";
        ...
        add () {
            this.$f7router.navigate({url: '/office/carList/apply/'});
        },
        load () {
            $preloader.show()
            if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                axios.get('/api/car/?approver=' + this.myUserId).then(res => {
            ...
            axios.get('/api/car/?creator=' + this.myUserId).then(res => {
