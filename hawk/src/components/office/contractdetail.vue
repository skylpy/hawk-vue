<template>
    <f7-page no-toolbar style="background: #f8f8f8">
        <f7-navbar  title="合同审批详情" back-link>
        </f7-navbar>
        ...
        <div style="overflow-x: hidden;">
            ...
            <div class="list-item  list-type">
                <label>我方单位名称：</label>
                <span>{{item.myCompanyName}}</span>
            </div>

            <div class="list-item list-type">
                <label>我方负责人：</label>
                <span>{{item.myUserName}}</span>
            </div>

            <div class="list-item list-type">
                <label>对方单位名称：</label>
                <span>{{item.otherCompanyName}}</span>
            </div>
...
import {getSignUrl, getPreviewUrl} from 'lib/aliyun.js';
export default {
    name: "journaldetail",
    ...
    methods: {
        ...
        promptpop(){
            ...
            $dialog.prompt('请填写驳回原因', '驳回原因', val => {
                ...
                data.comment = val;
                $preloader.show();
                axios.post('/api/contract/', data).then(() => {
                    $preloader.hide();
                    $alert('操作成功', '操作成功', () => {
                        this.$f7router.back()
                    })
        ...
        saveFile() {
            if (this.item.files && this.item.files.length === 0) return
            let update = JSON.parse(JSON.stringify(this.myCompany))
            let list = update.contracts && update.contracts.files || []
            update.contracts.files = list.concat(this.item.files)
