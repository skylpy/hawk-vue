<template>
    <f7-page no-toolbar style="background: #f8f8f8" @page:beforein="requestLoad">
        <f7-navbar  :title=title back-link>
            <f7-nav-right>
                <f7-link @click="$f7router.navigate('/office/paymentbxmgr/')">新建</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <div class="wrapper" v-if="myapprove && isSuperOrAdmin">
            ...
            <div v-show="tab=='pendingapproval'" class="payment-content">
                ...
                <div class="approval-list" v-for="(item,index) in pendapproval" :key="index" @click="godateil(item)">
                    ...
                    <div class="payment-right">
                        <li class="payment-right-title">
                            <span>{{who(item.applicant) && who(item.applicant).name}}提交的付款</span>
                        </li>
                        <li class="payment-right-des">
                            <span>付款的金额（元）：{{item.allmoney}}</span>
                        </li>
                        <li class="payment-right-des">
                            <span>付款方式：{{item.types}}</span>
                        </li>
                        <li class="payment-right-des">
                            <span>付款日期：{{moment(item.time).format('YYYY-MM-DD')}}</span>
                        </li>
                        <li class="payment-right-des">
                            <span>创建时间：{{new Date(item.createdAt).toHawkString()}}</span>
                        </li>
                        <li class="payment-right-pstate" v-if="item.status == '待审批'">
                            <span>{{item.status}}</span>
                        </li>
                        <li class="payment-right-state" v-if="item.status == '审批通过'">
                            <span>{{item.status}}</span>
                        </li>
                        <li class="payment-right-rstate" v-if="item.status == '已拒绝'">
            ...
            <div v-show="tab=='myapprovel'" class="payment-content" >
        ...
        <div class="wrapper" v-if="mysender && isSuperOrAdmin">
            ...
            <div v-show="tabs=='total'" class="payment-content">
            ...
            <div v-show="tabs=='notread'" class="payment-content" >
    ...
    export default {
        ...
        methods: {
            ...
            sendreader(item){
                ...
                axios.put('api/paymentmgr/mysender/' + item._id, item).then(() => {
                    this.nobusy();
            ...
            noreadLoad(){
                ...
                // axios.get('api/paymentmgr/mysender?q=' + JSON.stringify(filters)).then(resp => {
                //
            ...
            mysponsorLoad(){
                ...
                let filters = {};
                axios.get('api/paymentmgr/mysponsor?q=' + JSON.stringify(filters)).then(resp => {
            ...
            loading() {
                ...
                axios.get('api/paymentmgr?q=' + JSON.stringify(filters)).then(resp => {
