<template>
    <f7-page @page:beforein="load">
        <f7-navbar  title="用章申请" back-link>
            <f7-nav-right>
            </f7-nav-right>
        </f7-navbar>

        <div class="content">
            <div v-show="tab=='sealList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的用章申请</span>
                        </li>
                        <li class="expense-right-des">
                            <span>用印事由：{{item.result}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>印章类型：{{item.type}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>文件名称：{{item.name}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>文件份数：{{item.number}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>用印日期：{{moment(item.time).format('YYYY-MM-DD')}}</span>
                        </li>
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="sealList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>用章审批</span>
                </div>
                <div id="sealApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            changeClick(index){
                let obj1 = document.getElementById('sealList');
                let obj2 = document.getElementById('sealApply');
                if (index == 1){
                    this.tab = "sealList";
            ...
            add () {
                this.$f7router.navigate({url: '/office/sealList/apply/'});
            },
            load () {
                $preloader.show()
                if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                    axios.get('/api/seal/?approver=' + this.myUserId).then(res => {
                ...
                axios.get('/api/seal/?creator=' + this.myUserId).then(res => {
