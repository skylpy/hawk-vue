<template>
    <f7-page @page:reinit="load">
        <f7-navbar title="付款" back-link>
            <f7-nav-right>
                <f7-link @click="$f7router.navigate('/office/payment/')">新建</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <f7-list style="background-color:aliceblue; margin-top:5px">
            ...
            <f7-list-item style="display:inline-block">
                ...
                <f7-chip v-if="params.paymentType != '全部'" slot="inner" outline :text="params.paymentType" color="blue" style="margin-right: 5px"></f7-chip>
        ...
        <f7-block-title>付款申请单列表</f7-block-title>
                ...
                <f7-list>
                    ...
                    >
                        <select name="paymentType">
                            ...
                            <option
                                v-for="(t, index) in myCompany.paymentType"
                                :key="index"
                                :value="t"
                            >
                                {{t}}
...
export default {
    ...
    mounted() {
        ...
        $('select[name="paymentType"]').change(() => {
            this.params.paymentType = $('select[name="paymentType"] option:checked').attr('value');
            this.$f7.popover.close('.smart-select-popover');
        });
    ...
    methods: {
        ...
        goto(payment) {
            this.records = [];
            this.$f7router.navigate('/office/payment/', {
                props: {
                    lastPage: '付款',
                    reference: payment
                }
        ...
        load() {
            ...
            if (this.params.paymentType && this.params.paymentType != '全部') { content.paymentType = this.params.paymentType }
