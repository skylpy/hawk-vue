<template>
    <f7-page @page:beforein="load">
        ...
        <div class="content">
            <div v-show="tab=='contractList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的合同审批</span>
                        </li>
                        <li class="expense-right-des">
                            <span>合同名称：{{item.contract}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>合同编号：{{item.contractNo}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>签约时间：{{moment(item.time).format('YYYY-MM-DD')}}</span>
                        </li>
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="contractList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>审批列表</span>
                </div>
                <div id="contractApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
export default {
    ...
    methods: {
        moment,
        changeClick(index){
            let obj1 = document.getElementById('contractList');
            let obj2 = document.getElementById('contractApply');
            if (index == 1){
                this.tab = "contractList";
                obj1.style.color = "#01B38B";
                obj2.style.color = "#CDCDCD";
                $('#img1').attr('src',this.icon_employee_green);
                $('#img2').attr('src',this.icon_my_task_gray);
            } else if (index == 2){
                this.tab = "contractApply";
        ...
        add () {
            this.$f7router.navigate({url: '/office/contractList/apply/'});
        },
        load () {
            $preloader.show()
            if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                axios.get('/api/contract/?approver=' + this.myUserId).then(res => {
            ...
            axios.get('/api/contract/?creator=' + this.myUserId).then(res => {
