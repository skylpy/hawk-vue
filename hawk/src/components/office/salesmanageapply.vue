        ...
        <div>
            <div class="list-item clear-float">
                ...
                <div style="float: right">
                    <input class="item-input"
                           placeholder="请输入"
                           :value="formData.sale"
                           @input="formData.sale = $event.target.value"
                           validate
                           type="tel"
            ...
            <div class="list-item clear-float">
                ...
                <div style="float: right">
                    <input class="item-input"
                           placeholder="请输入"
                           :value="formData.tsale"
                           @input="formData.tsale = $event.target.value"
                           validate
                           type="tel"
                           pattern = "^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$"
                           error-message = "请输入正确数字"/>
            ...
            <div class="list-item clear-float">
                ...
                <div style="float: right">
                    <input class="item-input"
                           placeholder="请输入"
                           :value="formData.msale"
                           @input="formData.msale = $event.target.value"
                           validate
                           type="tel"
                           pattern = "^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$"
                           error-message = "请输入正确数字"/>
            ...
            <div class="list-item clear-float textarea-item">
                <span class="item-icon">*</span>
                <label>销售总结</label>
                <textarea  placeholder="请输入销售总结"
                           :value="formData.summary"
                           @input="formData.summary = $event.target.value">
                </textarea>
            </div>
            ...
            <div class="list-item clear-float textarea-item">
                <span class="item-icon">*</span>
                <label>明日销售计划</label>
                <textarea  placeholder="请输入明日销售计划"
                           :value="formData.plan"
...
export default {
    ...
    methods: {
        ...
        submit () {
            ...
            if (!/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/.test(this.formData.sale)) {
                error('请输入今日销售（数字）')
            }
            else if (this.formData.tsale && !/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/.test(this.formData.tsale)) {
                error('请输入月累计销售（数字）')
            }
            else if (!/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/.test(this.formData.msale)) {
                error('请输入本月目标（数字）')
            }
            else if (!this.formData.summary) {
                error('请输入销售总结')
            }
            else if (!this.formData.plan) {
                error('请输入明日销售计划')
            }
                ...
                axios.put('/api/salesmanage/', data).then(resp=>{
                    this.$f7router.back()
                }).catch(err => {
