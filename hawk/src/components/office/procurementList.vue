<template>
    <f7-page no-toolbar style="background: #f8f8f8" @page:beforein="requestLoad">
        <f7-navbar  :title=title back-link>
            <f7-nav-right>
                <f7-link @click="$f7router.navigate('/office/procurementbxmgr/')">新建</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <div class="wrapper" v-if="myapprove && isSuperOrAdmin">
            ...
            <div v-show="tab=='pendingapproval'" class="procurement-content">
                ...
                <div class="approval-list" v-for="(item,index) in pendapproval" :key="index" @click="godateil(item)">
                    ...
                    <div class="procurement-right">
                        <li class="procurement-right-title">
                            <span>{{who(item.applicant) && who(item.applicant).name}}提交的采购</span>
                        </li>
                        <li class="procurement-right-des">
                            <span>申请事由：{{item.result}}</span>
                        </li>
                        <li class="procurement-right-des">
                            <span>期望交付日期：{{moment(item.time).format('YYYY-MM-DD')}}</span>
                        </li>
                        <li class="procurement-right-des">
                            <span>采购的总金额（元）：{{item.allmoney}}</span>
                        </li>
                        <li class="procurement-right-des">
                            <span>创建时间：{{new Date(item.createdAt).toHawkString()}}</span>
                        </li>
                        <li class="procurement-right-pstate" v-if="item.status == '待审批'">
                            <span>{{item.status}}</span>
                        </li>
                        <li class="procurement-right-state" v-if="item.status == '审批通过'">
                            <span>{{item.status}}</span>
                        </li>
                        <li class="procurement-right-rstate" v-if="item.status == '已拒绝'">
            ...
            <div v-show="tab=='myapprovel'" class="procurement-content" >
        ...
        <div class="wrapper" v-if="mysender && isSuperOrAdmin">
            ...
            <div v-show="tabs=='total'" class="procurement-content">
            ...
            <div v-show="tabs=='notread'" class="procurement-content" >
    ...
    export default {
        ...
        methods: {
            ...
            sendreader(item){
                ...
                axios.put('api/procurement/mysender/' + item._id, item).then(() => {
                    this.nobusy();
            ...
            noreadLoad(){
                ...
                // axios.get('api/procurement/mysender?q=' + JSON.stringify(filters)).then(resp => {
                //
            ...
            mysponsorLoad(){
                ...
                let filters = {};
                axios.get('api/procurement/mysponsor?q=' + JSON.stringify(filters)).then(resp => {
            ...
            loading() {
                ...
                axios.get('api/procurement?q=' + JSON.stringify(filters)).then(resp => {
