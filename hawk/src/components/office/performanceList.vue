<template>
    <f7-page @page:beforein="load">
        ...
        <div class="content">
            <div v-show="tab=='performanceList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的绩效报告</span>
                        </li>
                        <li class="expense-right-des">
                            <span>上月工作内容：{{item.lastContent}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>上月完成情况：{{item.lastSchedule}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>完成率：{{item.finishing}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>上月工作自评：{{item.selfAssessment}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>本月工作目标：{{item.content}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>本月工作计划：{{item.plan}}</span>
                        </li>
            ...
            <div v-show="tab=='performanceApply'">
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="performanceList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>绩效审批</span>
                </div>
                <div id="performanceApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            changeClick(index){
                let obj1 = document.getElementById('performanceList');
                let obj2 = document.getElementById('performanceApply');
                if (index == 1){
                    this.tab = "performanceList";
                    obj1.style.color = "#01B38B";
                    obj2.style.color = "#CDCDCD";
                    $('#img1').attr('src',this.icon_employee_green);
                    $('#img2').attr('src',this.icon_my_task_gray);
                } else if (index == 2){
                    this.tab = "performanceApply";
            ...
            add () {
                this.$f7router.navigate({url: '/office/performanceList/apply/'});
            },
            load () {
                $preloader.show()
                if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                    axios.get('/api/performance/?approver=' + this.myUserId).then(res => {
                ...
                axios.get('/api/performance/?creator=' + this.myUserId).then(res => {
