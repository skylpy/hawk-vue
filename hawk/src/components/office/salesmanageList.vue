<template>
    <f7-page @page:beforein="load">
        <f7-navbar  title="销售业绩" back-link>
            <f7-nav-right>
            </f7-nav-right>
        </f7-navbar>

        <div class="content">
            <div v-show="tab=='salesmanageList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的销售业绩</span>
                        </li>
                        <li class="expense-right-des">
                            <span>今日销售（元）：{{item.sale}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>本月目标（元）：{{item.msale}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>销售总结：{{item.summary}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>明日销售计划：{{item.plan}}</span>
                        </li>
            ...
            <div v-show="tab=='salesmanageApply'">
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="salesmanageList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>销售审批</span>
                </div>
                <div id="salesmanageApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
export default {
    ...
    methods: {
        moment,
        changeClick(index){
            let obj1 = document.getElementById('salesmanageList');
            let obj2 = document.getElementById('salesmanageApply');
            if (index == 1){
                this.tab = "salesmanageList";
                obj1.style.color = "#01B38B";
                obj2.style.color = "#CDCDCD";
                $('#img1').attr('src',this.icon_employee_green);
                $('#img2').attr('src',this.icon_my_task_gray);
            } else if (index == 2){
                this.tab = "salesmanageApply";
        ...
        add () {
            this.$f7router.navigate({url: '/office/salesmanageList/apply/'});
        },
        load () {
            $preloader.show()
            if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                axios.get('/api/salesmanage/?approver=' + this.myUserId).then(res => {
            ...
            axios.get('/api/salesmanage/?creator=' + this.myUserId).then(res => {
