<template>
    <f7-page @page:reinit="load">
        <f7-navbar title="采购申请" back-link>
            <f7-nav-right>
                <f7-link @click="$f7router.navigate('/office/procurement/')">新建</f7-link>
            </f7-nav-right>
        </f7-navbar>
        ...
        <f7-list style="background-color:aliceblue; margin-top:5px">
            ...
            <f7-list-item style="display:inline-block">
                ...
                <f7-chip v-if="params.procurementType != '全部'" slot="inner" outline :text="params.procurementType" color="blue" style="margin-right: 5px"></f7-chip>
        ...
        <f7-block-title>采购单列表</f7-block-title>
        ...
        <f7-list v-else media-list>
            ...
            >
                <div slot="after-title" style="margin-left:20px">
                    <f7-chip
                        outline
                        :text="r.content.procurementType"
                    ></f7-chip>
                    <f7-chip
                        outline
                ...
                <f7-list>
                    ...
                    >
                        <select name="procurementType">
                            ...
                            <option
                                v-for="(t, index) in myCompany.procurementType"
                                :key="index"
                                :value="t"
                            >
                                {{t}}
...
export default {
    ...
    mounted() {
        ...
        $('select[name="procurementType"]').change(() => {
            this.params.procurementType = $('select[name="procurementType"] option:checked').attr('value');
            this.$f7.popover.close('.smart-select-popover');
        });
    ...
    methods: {
        ...
        goto(procurement) {
            this.records = [];
            this.$f7router.navigate('/office/procurement/', {
                props: {
                    lastPage: '采购申请',
        ...
        load() {
            ...
            if (this.params.procurementType && this.params.procurementType != '全部') { content.procurementType = this.params.procurementType }
