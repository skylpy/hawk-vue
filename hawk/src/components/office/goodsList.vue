<template>
    <f7-page @page:beforein="load">
        ...
        <div class="content">
            <div v-show="tab=='goodsList'" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
                <div class="approval-list" v-for="(item,index) in approveList" :key="index" @click="goDetail(item)">
                    ...
                    <div class="expense-right">
                        <li class="expense-right-title">
                            <span>{{who(item.creator) && who(item.creator).name}}提交的物品领用</span>
                        </li>
                        <li class="expense-right-des">
                            <span>用途：{{item.use}}</span>
                        </li>
                        <li class="expense-right-des">
                            <span>物品名称：{{item.name}}</span>
                        </li>
                        <li class="expense-right-des">
        ...
        <footer class="footer" v-if="superAdmin._id === myProfile._id || isAdministrative || isFinance">
            <div class="clearfix">
                <div id="goodsList" class="divclsaa" @click="changeClick(1)" >
                    <div class="item-image">
                        <img id="img1" :src=icon_employee_gray width="25px" height="25px">
                    </div>
                    <span>领用审批</span>
                </div>
                <div id="goodsApply" class="divclsaa" @click="changeClick(2)" >
                    <div class="item-image">
...
<script>
    ...
    export default {
        ...
        methods: {
            moment,
            changeClick(index){
                let obj1 = document.getElementById('goodsList');
                let obj2 = document.getElementById('goodsApply');
                if (index == 1){
                    this.tab = "goodsList";
            ...
            add () {
                this.$f7router.navigate({url: '/office/goodsList/apply/'});
            },
            load () {
                $preloader.show()
                if (this.superAdmin._id === this.myProfile._id || this.isAdministrative || this.isFinance) {
                    axios.get('/api/goods/?approver=' + this.myUserId).then(res => {
                ...
                axios.get('/api/goods/?creator=' + this.myUserId).then(res => {
