<template>
    <f7-page>
        <f7-navbar title="车辆类型" back-link>
            <f7-nav-right>
...
<script>
    ...
    export default {
        ...
        methods: {
            addType () {
                $prompt('请输入车辆类型', '新建车辆类型', (name) => {
                    if (!name) {
                        $alert('车辆类型名称不能为空', '操作错误');
                        return;
                    }
                    let type = Object.assign([], this.list)
                    type.push(name)

                    axios[this.isCreate ? 'put' : 'post']('/api/car/type/', {type: type}).then(resp=>{
                        this.list.push(name)
                        this.isCreate = false
                    }).catch(err => {
                        console.error(err);
