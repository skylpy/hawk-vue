<template>
    <f7-page>
        <f7-navbar title="付款方式" back-link>
            <f7-nav-right>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                list: ['银行卡', '现金', '汇票', '电汇', '贷记', '支票', '其他'],
                isCreate: true
            }
        },
        methods: {
            addType () {
                $prompt('请输入付款方式', '新建付款方式', (name) => {
                    if (!name) {
                        $alert('付款方式名称不能为空', '操作错误');
                        return;
                    }
                    let type = Object.assign([], this.list)
                    type.push(name)

                    axios[this.isCreate ? 'put' : 'post']('/api/paymentset', {type: type}).then(resp=>{
                        this.list.push(name)
                        this.isCreate = false
                    }).catch(err => {
                        console.error(err);
